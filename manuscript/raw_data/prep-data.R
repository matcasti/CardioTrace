...
fig1a <- ggplot(data, aes(time)) +
  geom_line(aes(y = elitehrv, col = "EliteHRV"), linewidth = 1) +
  geom_line(aes(y = cardiotrace, col = "CardioTrace")) +
  scale_color_manual(values = c(EliteHRV = "orange", CardioTrace = "black")) +
  theme_classic() +
...
fig1b <- ggplot(data, aes(elitehrv, cardiotrace, col = "Data point")) +
  ...
  scale_color_manual(values = c("Data point" = "orange")) +
  theme_classic() +
...
ggsave("manuscript/figure-2.png", fig1, device = "png", scale = 4, units = "px",
